...
""", unsafe_allow_html=True)
    ...
    # ── Export — prominent, always visible above tabs ────────────────────
    st.markdown("""
<div style='background:linear-gradient(135deg,#1F3A5F 0%,#3A7AFE 100%);
border-radius:12px;padding:18px 24px;margin:16px 0;direction:rtl;
display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px'>
  <div>
    <div style='color:#fff;font-size:17px;font-weight:900'>📥 הפקת דוחות ויצוא</div>
    <div style='color:#93c5fd;font-size:12px;margin-top:3px'>
      דוח HTML מעוצב · Jupyter Notebook · NotebookLM · CSV
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    # ── Analysis tabs ──────────────────────────────────────────────────────
    t_charts, t_costs, t_table, t_whatif = st.tabs([
        "📈 גרפים",
        "💰 עלויות",
        "📋 טבלה",
        "🔀 What-If",
