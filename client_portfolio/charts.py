...
_BASE = dict(
    ...
    margin=dict(l=30, r=30, t=55, b=120),
...
def chart_by_manager(df):
    ...
    fig.update_layout(**_BASE, title=_title("פיזור בין מנהלים"), height=420,
...
def chart_stocks_bonds(df):
    ...
    fig.update_layout(**_BASE, title=_title('מניות · אג"ח · לא-סחיר'), barmode="stack",
                      height=380, yaxis=dict(ticksuffix="%",range=[0,105],gridcolor="#E5E7EB",automargin=True),
...
def chart_foreign_domestic(df):
    ...
    fig.update_layout(**_BASE,title=_title('חו"ל vs ישראל'),height=400,margin=dict(l=20,r=20,t=55,b=110))
    return fig
...
def chart_fx_ils(df):
    ...
    fig = go.Figure(go.Pie(labels=['מט"ח',"שקל"],values=[fx,ils],hole=0.5,
        ...
        textinfo="label+percent",textfont=dict(size=12),insidetextorientation="auto"))
    fig.update_layout(**_BASE,title=_title('מט"ח vs שקל'),height=400,margin=dict(l=20,r=20,t=55,b=110))
    return fig
...
def chart_asset_breakdown(df):
    ...
    fig.update_layout(**_BASE,title=_title("פיזור לפי סוג מוצר"),
                      height=440,margin=dict(l=20,r=20,t=55,b=120))
    return fig
...
def chart_annuity_capital(df, manual_annuity_pct=None):
    ...
    fig.update_layout(**_BASE,title=_title(f"קצבה vs הון{sub}"),
                      height=420,margin=dict(l=20,r=20,t=55,b=110))
    return fig
